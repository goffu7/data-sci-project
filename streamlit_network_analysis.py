...
def load_network_from_file(uploaded_file, file_format):
    ...
    try:
        if file_format == 'CSV':
            df = pd.read_csv(uploaded_file, engine="python", sep=r",|\s+", on_bad_lines="skip")
            
            # ตรวจสอบว่ามีคอลัมน์อย่างน้อย 2 คอลัมน์
            if len(df.columns) < 2:
            ...
            # ลบแถวที่มีค่า NaN ในคอลัมน์ที่ใช้
...
class NetworkVisualizer:
    ...
    def create_interactive_network(
        ...
        node_size_range: Tuple[int, int] = (15, 70),
